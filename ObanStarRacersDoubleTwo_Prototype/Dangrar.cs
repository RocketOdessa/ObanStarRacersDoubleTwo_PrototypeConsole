...
namespace ObanStarRacersDoubleTwo_Prototype
{
    class Dangrar : Battle
    {
        ...
        public void HelloDangrar()
        {
            char[] arrayDangrar = "Hello young racer! You chose this map for playing.\nIt will be hard, but I think you can do this.\nThis is not story map, it's some DLC for this game".ToCharArray();
            for (int i = 0; i < arrayDangrar.Length; i++)
            {
                Console.Write(arrayDangrar[i]);
                Thread.Sleep(75);
            }
            Console.WriteLine();
            Console.WriteLine();
